...
class FavoriteHistoryCacheTest {

    private val dbDataSource = mockk<FavoriteDbDataSource>()
    private val memoryDataSource = mockk<FavoriteMemoryDataSource>()
    private val cache = FavoriteCacheImpl(dbDataSource, memoryDataSource)

    private val mockItems = listOf<FavoriteRelative>(
        mockk {
    ...
    fun `get list WHEN db & memory empty EXPECT success empty`() {
        val insertSlot = slot<List<Pair<ReleaseKey, FavoriteRelative>>>()
        val dbResult = listOf<FavoriteRelative>()
        val memoryResult = listOf<FavoriteRelative>()
        val expectResult = emptyList<FavoriteRelative>()
    ...
    fun `get list WHEN memory not empty EXPECT success get from memory`() {
        val memoryResult = listOf<FavoriteRelative>(mockk(), mockk())
