...
class ReleaseApiDataSourceImpl(
    ...
    override fun getOne(releaseId: Int?, releaseCode: String?): Single<Release> = Single
        .defer {
            ...
            releaseService.getOne(params)
        }
        .handleApiResponse()
        .map { releaseConverter.toDomain(it) }

    override fun getSome(ids: List<Int>?, codes: List<String>?): Single<List<Release>> = Single
        .defer {
